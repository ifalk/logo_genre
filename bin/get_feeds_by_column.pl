...
get_feeds_by_column.pl

=head1 USAGE

 perl get_feeds_by_column.pl --journal='lemonde' rss html page   

=head1 DESCRIPTION

Parses the html page given as input and collects the links and associated column names (categories), dates and other relevant information.
...
my %GET_FEED4JOURNAL = (
  ...
  'lalibre' => sub {
    ...
    my @next_feeds = ($feed_header->findnodes('following-sibling::h3'));

    foreach my $feed_h (@next_feeds) {
      my $column_name = $feed_h->textContent();

      $column_name =~ s{ \r }{}xmsg;
      $column_name =~ s{ \s+ }{ }xms;
      $column_name =~ s{ \A \s+ }{}xms;
      $column_name =~ s{ \s+ \z }{}xms;
      ...
      last if ($column_name =~ m{ conditions \s d'utilisation }xmsi);

      

      my $next_table = ($feed_h->findnodes('following-sibling::table'))[0];
      my @tr = $next_table->findnodes('tr');

      ### first column is column (category),
      ### second contains link
      foreach my $row (@tr) {
	my @td = $row->findnodes('td');

	my $cat = $td[0]->textContent();
	next unless ($cat);
	next if ($cat =~ m{ La \s Une }xms);
	$cat =~ s{ \r }{}xmsg;
	$cat =~ s{ \s+ }{ }xms;
	$cat =~ s{ \A \s+ }{}xms;
	$cat =~ s{ \s+ \z }{}xms;

	my $link = ($td[1]->findnodes('a'))[0]->getAttribute('href');

	$cat = join(', ', $column_name, $cat);
	$feeds->{$link}->{$cat}++;
	print STDERR $cat, "\n";
      }
